...
{% block content %}
    ...
    {% set channel = request.get('channel','string','all') %}
    ...
    <div class="payment wrap">
        ...
        <div class="channel">
            {% if pay_provider.alipay.enabled == 1 and channel in ['all','alipay'] %}
                <a class="alipay btn-pay" href="javascript:" data-channel="alipay">{{ image('home/img/alipay.png') }}</a>
            {% endif %}
            {% if pay_provider.wxpay.enabled == 1 and channel in ['all','wxpay'] %}
