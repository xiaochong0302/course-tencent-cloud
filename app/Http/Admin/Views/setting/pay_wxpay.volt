{% set virtual_pay = wxpay.virtual_pay %}
...
<form class="layui-form kg-form" method="POST" action="{{ url({'for':'admin.setting.pay'}) }}">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <select name="service_rate" lay-verify="number">
                {% for value in 0..30 %}
                    {% set selected = (value == wxpay.service_rate) ? 'selected' : '' %}
    ...
    <div class="layui-form-item">
        <label class="layui-form-label">微信支付公钥ID</label>
        <div class="layui-input-block">
            <input class="layui-input" type="text" name="wechat_public_key_id" value="{{ wxpay.wechat_public_key_id }}" lay-verify="required">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">APIv3密钥</label>
        <div class="layui-input-block">
            <input class="layui-input" type="text" name="mch_secret_key" value="{{ wxpay.mch_secret_key }}" lay-verify="required">
        </div>
    </div>
    <div class="layui-form-item">
