        ...
        <tr>
            ...
            <td>
                {% if refund.apply_note %}
                    <p class="layui-elip">退款原因：<a href="javascript:" title="{{ refund.apply_note }}">{{ refund.apply_note }}</a></p>
                {% endif %}
                {% if refund.review_note %}
                    <p class="layui-elip">审核意见：<a href="javascript:" title="{{ refund.review_note }}">{{ refund.review_note }}</p>
                {% endif %}
                {% if refund.error_note %}
                    {% set error = refund.error_note|json_decode %}
                    {% set title = 'code: %s, message: %s'|format(error.code,error.message) %}
                    <p class="layui-elip">错误信息：<a href="javascript:" title="{{ title }}">{{ error.message }}</a></p>
                {% endif %}
            </td>
