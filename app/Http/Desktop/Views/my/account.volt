...
{% block content %}
    ...
    {% set edit_pwd_url = url({'for':'desktop.account.edit_pwd'}) %}
    {% set edit_phone_url = url({'for':'desktop.account.edit_phone'}) %}
    {% set edit_email_url = url({'for':'desktop.account.edit_email'}) %}
...
{% block include_js %}
    ...
    {{ js_include('desktop/js/my.account.js') }}
