...
trait Client
{
    ...
    protected function isHarmonyMobile(?string $userAgent = null): bool
    {
        if (!$userAgent) return false;

        $userAgent = strtolower($userAgent);

        if (!str_contains($userAgent, 'harmony')) return false;

        $keywords = ['mobile', 'phone', 'tablet'];

        return array_any($keywords, fn($keyword) => str_contains($userAgent, $keyword));
    }
